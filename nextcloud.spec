...
# TODO: Review, document and upstream patches
...
BuildRequires:       php-composer(symfony/console) < 4.0.0
BuildRequires:       php-composer(symfony/event-dispatcher) >= 3.0.0
BuildRequires:       php-composer(symfony/event-dispatcher) < 4.0.0
BuildRequires:       php-composer(symfony/routing) >= 3.0.0
BuildRequires:       php-composer(symfony/routing) < 4.0.0
BuildRequires:       php-composer(symfony/process) >= 3.0.0
BuildRequires:       php-composer(symfony/process) < 4.0.0
...
# pulls in guzzle/guzzle, psr/log and mikemccabe/json-patch-php
# https://github.com/nextcloud/3rdparty/blob/v12.0.2/rackspace/php-opencloud/composer.json
Requires:       php-composer(rackspace/php-opencloud) >= 1.16.0
Requires:       php-composer(rackspace/php-opencloud) < 2.0
Requires:       php-composer(guzzle/guzzle) >= 3.8
Requires:       php-composer(guzzle/guzzle) < 4
Requires:       php-composer(psr/log) >= 1.0
Requires:       php-composer(psr/log) < 2
Requires:       php-composer(mikemccabe/json-patch-php) >= 0.1
Requires:       php-composer(mikemccabe/json-patch-php) < 1
...
# https://github.com/nextcloud/server/blob/v12.0.2/apps/files_external/3rdparty/composer.json
#"icewind/smb": "2.0.0"
...
# These next dependencies are not listed in composer but manually dropped in place
Requires:       php-pear(pear.dropbox-php.com/Dropbox)
# Note: Google Drive external storage 1.1.8 has BC breaks
Requires:       php-composer(google/apiclient) >= 1.1.7
Requires:       php-composer(google/apiclient) < 1.1.8
Requires:       php-composer(aws/aws-sdk-php) >= 2.7.5
...
# External PHP libs required by user_saml app
# https://github.com/nextcloud/user_saml/blob/v12.0.0beta2/3rdparty/composer.json
...
# TODO: Update Provides declarations for bundled js libs

# js libs bundled in core
...
## js libs bundled in files_pdfviewer app
Provides: bundled(js-pdfjs) = 1.1.469

## js libs bundled in files_theming app
Provides: bundled(js-jscolor) = 2.0.4

## js libs bundled in federatedfilesharing app
Provides: bundled(js-gs-share)

## ls libs bundled in files_texteditor app
...
%package fpm
Summary:    Php-fpm integration without local Nginx for NextCloud
...
# https://github.com/nextcloud/3rdparty/tree/v12.0.2
pushd 3rdparty
...
rm -r guzzle/guzzle
rm -r guzzlehttp/{guzzle,ringphp,streams}
rm -r icewind/{streams,seaerchdav}
...
rm -r apps/files_external/3rdparty/{Dropbox,aws-sdk-php,google-api-php-client,icewind,composer*}

# create autoloader for files_external app
# from https://github.com/nextcloud/server/blob/v12.0.2/apps/files_external/3rdparty/composer.json
...
%changelog
* Tue Aug 22 2017 Christian Glombek <christian.glombekg@rwth-aachen.de> - 12.0.2-1
- Update to 12.0.2
- Updated dependencies
- Added fpm package for use with external nginx server
